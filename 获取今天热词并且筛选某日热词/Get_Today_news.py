...
cx.text_factory = str
cu = cx.cursor()
# cu.execute("create table Day(Title char(100) ,Link char(100),Accurate_time char(20),Content char(5000),primary key (Title,Link,Accurate_time,Content))")
	...
	def getNewsFromRollPage(self,site):
		...
		for i in m:
			try:
				...
				# 	f.write(txtSource)
				try:
					Tup = (str(newsTitle),str(newsUrl),str(newsTime),str(strText))
					try:
						cu.execute("insert into Day values (?,?,?,?)", Tup)
					except:
						pass
					cx.commit()
				except:
